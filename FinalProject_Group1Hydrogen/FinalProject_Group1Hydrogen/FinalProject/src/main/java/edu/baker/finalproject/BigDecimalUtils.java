...
//        return new BigDecimal(Math.pow(base.doubleValue(), (int)ipower), mc);
    ...
    static final BigDecimal exp(BigDecimal x, MathContext mc) {
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            // Use exp(-x) = 1 / exp(x) for negative x to improve precision
        ...
        BigDecimal epsilon = BigDecimal.ONE.scaleByPowerOfTen(-mc.getPrecision()); // Tolerance for convergence
     ...
     * @param x The exponent
     * @param mc The MathContext for precision control
     * @return The base-10 logarithm of the input BigDecimal with the specified
     * precision.
     * @author Yordanos Shiferaw
     */
    static final BigDecimal log10(BigDecimal x, MathContext mc) {
        ...
        BigDecimal result = log(x, mc2).divide(log(BigDecimal.TEN, mc2), RoundingMode.HALF_UP);
        return result.round(mc).setScale(mc.getPrecision());
    }
    ...
    static final BigDecimal atan2(BigDecimal y, BigDecimal x, MathContext mc) {
        BigDecimal HALF_PI = pi(mc).divide(BigDecimal.valueOf(2), MathContext.DECIMAL128);

        // Check if x is zero
        if (x.compareTo(BigDecimal.ZERO) == 0) {
            // Return π/2 if y > 0, otherwise -π/2
            return (y.compareTo(BigDecimal.ZERO) > 0) ? HALF_PI
                    : (y.compareTo(BigDecimal.ZERO) < 0) ? HALF_PI.negate()
                    : BigDecimal.ZERO;
        }
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            if (y.compareTo(BigDecimal.ZERO) < 0) {
                atanValue = atanValue.subtract(pi(mc));
            } else {
                atanValue = atanValue.add(pi(mc));
            }
        }
